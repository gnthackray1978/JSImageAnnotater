...
var TreeUI = function (modelCode, callback) {
    ...
    //http://localhost:59747/Images/icons/24x24/plus.jpg
...
TreeUI.prototype = {
    ...
    DrawLabel:function(x,y,x2,y2,text,backcolour){

        this.ctx.beginPath(); 
        this.ctx.rect(x, y, Math.abs(x2), Math.abs(y2));
        ...
        while (idx < 150) {
            ...
            this.ctx.font = idx+"pt Helvetica";

            var mo = this.ctx.measureText(text);

            if (mo.width > x2 || mo.height > y2) {
                idx--;
                break;
            }
        ...
        this.ctx.fillText(text, x+ (x2 / 2), y+ (y2 / 2));
